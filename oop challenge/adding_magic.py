...
class Fighter: #the fighter class being set up
    def __init__(self,name,health,energy,mana,weapon,armour,strength,speed,magic):
        self.name = name
        self.health = health
        self.energy = energy
        self.mana = mana
    ...
    def set_magic(self):
        while True:
            ...
            try:
                ...
                print(f'Your stat points are now at {self.stat_points} and your magic is now at {self.magic}')
                break
            except ValueError:
                print('please enter a number.')        
    ...
    def choose_magic(self):
        spells = {
            '1' : {'name':'hearty sacrifice'},
            '2' : {'name':'fireball','damage':35},
            '3' : {'name':'circle of life'}
        }
    ...
    def fireball(self):
        global distance
        if self.magic < 4:
            print("you don't have eneough magic")
        else:
            magic_power = 30 + (self.magic*3)
            self.mana -= 40
            if distance <= 5:
                self.health -= magic_power
            print('Attack power:', magic_power) #shows player the attack power
            return magic_power
        

    def magic_defend(self,magic_power):
        if self.armour > 0: #checks if target has armour
            self.armour -= round(magic_power*0.5) #If target has armour then armour takes 50%
            self.health -= round(magic_power*0.3) #target then takes 30% damage to health and 20% damage is completely mitgated
            print(self.name, 'took', round(magic_power*0.3), 'damage')
        else:
            self.health -= magic_power #if target is in range and had no armour the target takes all the damage to the health
            print(self.name, 'took', magic_power, 'damage')
    ...
    def self_heal(self):
        if self.magic < 2:
            ...
            healing_power = 20 + round(self.magic*4)
            self.health += healing_power
            self.mana -= 20
            if self.health > self.max_health: #checks if players health is over max health
                self.health = self.max_health
            print(f'you have healed ',{healing_power},'health and now have',{self.health})

    def hearty_sacrifice(self):
        while True:
            if self.magic < 3:
                print("you don't have enough magic power")
            else:
                sacrifice = input('How much of your body do you offer?')
                try:
                    int(sacrifice)
                except ValueError:
                    print('enter a number')
                    continue
                self.health -= int(sacrifice)
                self.mana += int(sacrifice)
                print(f'you have sacrificed {sacrifice} hp')
                break
...
you = Fighter('You', 100,100,80, 30, 100, 0, 0 ,0)
troll = Fighter('Troll',150,100,80, 40, 50, 6, 3, 1)
